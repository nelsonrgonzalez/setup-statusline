#!/usr/bin/env bash
# Claude Code statusline — Git Bash (Windows) version.
# Requires: Git for Windows (bash + GNU coreutils), jq.exe on PATH, Windows Terminal for ANSI colors.
...
# Git for Windows ships GNU coreutils — use `date -d @<epoch>` like Linux.
...
# Uses a while-loop instead of mapfile for bash 3.2 compatibility (Git Bash ships bash 3.2).
_f=()
