#!/bin/bash
# Claude Code statusline — WSL (Windows Subsystem for Linux) version.
...
case "$model_raw" in
  *opus*)  model_color="\033[01;33m" ;;   # gold
  *haiku*) model_color="\033[01;32m" ;;   # green
  *)       model_color="\033[01;36m" ;;   # cyan (sonnet + default)
esac
model_badges=""
[ -n "$_think" ] && model_badges="${model_badges} \033[01;35m💡\033[00m"
[ -n "$_fast"  ] && model_badges="${model_badges} \033[01;37m⚡\033[00m"
case "$_effort" in
  max)    model_badges="${model_badges} \033[01;37m\033[41mmax\033[00m" ;;   # bold white on red bg
...
# Context bar: fill color, icon, and section background scale with usage %.
# Autocompact fires at ~83.5% (Claude Code reserves a 33k/16.5% buffer),
# so ≥80% is treated as the critical threshold — bright red background.
if [ -n "$ctx_pct" ]; then
  ctx_val=$(printf '%.0f' "$ctx_pct")
  if [ "$ctx_val" -ge 80 ]; then
    fill_color="\033[01;31m"; win_bg="\033[101m\033[01;37m"; bar_icon="⚠"   # critical — autocompact imminent
  elif [ "$ctx_val" -ge 75 ]; then
    fill_color="\033[01;31m"; win_bg="\033[40m\033[01;37m";  bar_icon="⛁"   # danger
  elif [ "$ctx_val" -ge 65 ]; then
    fill_color="\033[01;33m"; win_bg="\033[40m\033[01;37m";  bar_icon="⛁"   # warning
  else
    fill_color="\033[01;32m"; win_bg="\033[40m\033[01;37m";  bar_icon="⛀"   # healthy
  fi
else
  fill_color="\033[01;32m"; win_bg="\033[40m\033[01;37m"; bar_icon="⛀"
fi

# Current-turn token fields — only populated when cu="1" (current_usage present)
if [ -n "$cu" ]; then
...
# Cache efficiency: ratio of cache reads to total cache activity (reads + writes).
# Uses raw token counts (_cc/_cr), not the formatted display strings (cc_str/cr_str).
...
# Session total token strings (Σ = sum across all turns)
tin_str=$(fmt_num "$_tin");   [ -n "$tin_str"  ] && tin_str="Σ↓${tin_str}"
tout_str=$(fmt_num "$_tout"); [ -n "$tout_str" ] && tout_str="Σ↑${tout_str}"

# Cost with smart formatting (scale precision to magnitude; suppress when zero)
...
# Duration: API-only time / total wall-clock time (⧗ api╱wall)
dur_str=""
...
# 5h shows countdown + local reset time [↻Xh @HH:MM].
# 7d shows countdown only [↻Xd] — local time omitted intentionally (too far ahead to be useful).
rl_str=""
rl_color=""
...
# Context bar: icon + smooth bar + used/total/percent.
# Numbers always use a neutral dark background regardless of warning level —
# only the bar icon inherits the alarm background (set on line2 assembly below).
...
# Git status — runs in cwd from payload; shows branch+counts, or dim indicator if no repo
...
# Line 1: location | git branch | model + badges
line1="$location"
...
# Line 3: current-turn tokens | cache + efficiency | cost / duration / lines
line3=""
token_parts=""
...
# printf "%b" interprets the \033 escape sequences accumulated throughout as ANSI codes
printf "%b" "$output"
